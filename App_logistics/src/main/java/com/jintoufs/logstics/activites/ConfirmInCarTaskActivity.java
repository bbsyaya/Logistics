 ...
 */
@SuppressLint("NewApi")
public class ConfirmInCarTaskActivity extends BaseRfidActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mHeaderCode = (TextView) findViewById(R.id.header_code);
	...
	protected void onResume() {
		...
		Api.getInCarCashboxList(user.getKeyStr(), mHandler);
	}
	...
	public void save(View view) {
		...
		if(bException){
			...
			Api.confirmInCarTask(user.getKeyStr(), lstFound, mSaveHandler);
		}
	...
	private final AsyncHttpResponseHandler mSaveHandler = new JsonHttpResponseHandler() {
		...
		public void onSuccess(int arg0, Header[] arg1, JSONObject response) {
			try {
				...
				if (ajaxMsg.getCode().equals(AjaxMsg.SUCCESS)) {
					hideWaitDialog();
					AppContext.showToast("上车任务确认成功");
					ConfirmInCarTaskActivity.this.finish();
