...
import com.jintoufs.logstics.reader.ERROR;
import com.jintoufs.logstics.reader.HEAD;
import com.jintoufs.logstics.reader.MessageTran;
import com.jintoufs.logstics.reader.model.ISO180006BOperateTagBuffer;
import com.jintoufs.logstics.reader.model.InventoryBuffer;
import com.jintoufs.logstics.reader.model.OperateTagBuffer;
...
import com.jintoufs.logstics.reader.model.OperateTagBuffer.OperateTagMap;
...
//			throw new NullPointerException("mReaderHelper Or mContext is Null!");
	 ...
	 * ��ʾlog��
	 * @param strLog	log��Ϣ
	 * @param type		log�ȼ�(0x10:��ȷ, 0x11:����)  play
	 */
...
*/		
	...
	private void processSetWorkAntenna(MessageTran msgTran) {
    	...
    	byte []btAryData = msgTran.getAryData();
		String strErrorCode = "";
        String strCmd = "���ù������߳ɹ�,��ǰ��������: ����" + (m_curReaderSetting.btWorkAntenna + 1);
    ...
    private void processWriteTag(MessageTran msgTran) {
        ...
        if (btAryData.length == 0x01) {
    		...
    		appContext.setWriteCardInfo(-1, strLog);
            //writeLog(strLog, ERROR.FAIL);
        } else {
        	int nLen = btAryData.length;
    ...
    private void processRealTimeInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {		//���λ�ȡ��Ϊ�������
			strErrorCode = ERROR.format(btAryData[0]);
			String strLog = strCmd + "ʧ�ܣ�ʧ��ԭ�� " + strErrorCode;
			...
			runLoopInventroy();
		} else if (btAryData.length == 0x07) {	//���λ�ȡ�����
    ...
    private void processFastSwitchInventory(MessageTran msgTran) {
        ...
        if (btAryData.length == 0x01) {		//��ѯ��Ϊ�������
            ...
            runLoopFastSwitch();
        } else if (btAryData.length == 0x02) {		//��ѯ����г��ִ��󣬵�δ����
            strErrorCode = ERROR.format(btAryData[1]);
            String strLog = strCmd + "ʧ�ܣ�ʧ��ԭ�� " + strErrorCode + "--" + "����" + ((btAryData[0] & 0xFF) + 1);

            writeLog(strLog, ERROR.FAIL);
        } else if (btAryData.length == 0x07) {		//��ѯ�����
    ...
    /**
     * processCustomizedSessionTargetInventory �� processRealTimeInventory ����һ�¡�
     * @param msgTran	��Ϣ������
     */
    ...
    /**
     * processGetAndResetInventoryBuffer �� processGetInventoryBuffer ����һ�¡�
     * @param msgTran	��Ϣ������
     */
