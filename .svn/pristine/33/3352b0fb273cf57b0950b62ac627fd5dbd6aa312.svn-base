...
public class OperationView extends RelativeLayout {
	...
	public OperationView(Context context, AttributeSet attrs) {
		...
		if (!TextUtils.isEmpty(status) && status.toString().equals(Constants.ATM_TASK_RESULT_SUCCESS)) {
	...
	public void refreshStatus(Integer state){
		if (state.equals(Constants.ATM_TASK_RESULT_SUCCESS)) {
			mButton.setBackgroundResource(R.drawable.btn_radio_on);
		}else if(state.equals(Constants.ATM_TASK_RESULT_FAILURE)){
			mButton.setBackgroundResource(R.drawable.error);
		}else {
			mButton.setBackgroundResource(R.drawable.btn_radio_off);
		}
	}
